...
var getTypeStations = function (typeID, callback) {
  let typeKey = 'type:' + typeID + ':stations'
  // 拿到某类型的所有站
  client.smembersAsync(typeKey).then(function (stations) {
    // 过滤高安站, 并获取星系信息
    filterHighsecStations(stations, function (stationSolarSystems) {
      // 获取站内订单
      Promise.all(stationSolarSystems.map(function (stationSolarSystem) {
        let stationID = stationSolarSystem.station_id
        let stationTypeKey = 'station:' + stationID + ':type:' + typeID
...
var filterHighsecStations = function (stations, callback) {
...
var getTypeStationOrderPriceVolume = function (callback) {
  getTypeStations(typeID, function (stationOrders) {
...
var getTypeMostProfitableRoute = function (callback) {
  getTypeStationOrderPriceVolume(function (stationPrices) {
...
var getTypeRouteDetail = function (callback) {
  getTypeMostProfitableRoute(function (route) {
    var promises = []
    promises.push(
...
getTypeRouteDetail(function (route) {
  console.log(route)
})
